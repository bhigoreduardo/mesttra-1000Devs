...
public class Gerente {
	...
	private static Pessoa cadastrarPessoaFisica(String numeroConta, String agencia, String telefone, BigDecimal saldo,
	...
	private static Pessoa cadastrarPessoaJuridica(String numeroConta, String agencia, String telefone, BigDecimal saldo,
	...
	private static Pessoa[] obterCliente(Pessoa[] clientes, Scanner input, Log[] logs) {
		...
		case "1":
			...
			novoCliente = cadastrarPessoaFisica(numeroConta, agencia, telefone, saldo, limiteChequeEspecial, nome, cpf,
					idade);
			break;
		case "2":
			...
			novoCliente = cadastrarPessoaJuridica(numeroConta, agencia, telefone, saldo, limiteChequeEspecial, cnpj,
					socios, nomeFantasia);
			break;
		default:
			return null;
		...
		for (int i = 0; i < clientes.length; i++) {
			if (clientes[i] == null) {
				if (novoCliente != null) {
					...
					cadastrarLog(logs, String.format("Cliente Nr. Conta %s cadastrado na base de dados.", numeroConta));
		...
		cadastroMessage(clientes);

		return clientes;
	}

	public Pessoa[] cadastrarCliente(Pessoa[] clientes, Scanner input, Log[] logs) {

		System.out.println("\t\tCADASTRAR CLIENTE:");
		System.out.println("\t\t***********************************************");

		if (clientes[clientes.length - 1] != null) {
			System.err.println("\t\tQuantidade maxima de clientes cadastrados na base de dados atingida.");
			return null;
		}

		Pessoa[] novosClientes = obterCliente(clientes, input, logs);
